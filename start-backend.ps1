...
Write-Host "[i] Note: This requires images to be built by GitHub Actions" -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "`n[i] Container Status:" -ForegroundColor Cyan
    docker-compose -f docker-compose.ghcr.yml ps
    
    Write-Host "`n[i] Backend Services:" -ForegroundColor Green
